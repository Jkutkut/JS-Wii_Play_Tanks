...
function loadLevelmk2(lvl) {
    ...
    let getPos = (type, pos) => {return indexPos2pixelPos(...data2indexPos(type, pos))};

    let level = levelsJSON.levels[lvl];


    tank = new TankPlayer(...getPos("tank", level.player));
    ...
    for(let enemyTank of level.tanks) {
        AItanks.push(new enemyTanksConv[enemyTank.type](...getPos("tank", enemyTank.pos)));
    }
}

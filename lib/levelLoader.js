...
function loadLevel(lvl) {
    ...
    for (let wall of level.environment.walls) {
        ...
        if (typeof wall.x != "number" || typeof wall.y != "number") {
            ...
            walls.push(new WallByIndex(...getPos(wall), ...wSize));
        }
    }
    ...
    // console.log(...getPos(level.player))
    // tank = new TankPlayer(...indexPos2pixelPos(...data2indexPos(level.player).map(p => {p[0] += 0.5})));
    tank = new TankPlayer(... getPos(level.player));
    let extra = createVector(
        (typeof level.player.x == "number")? 0.5 * cubeSize.x: 0,
        (typeof level.player.y == "number")? 0.5 * cubeSize.y: 0
    );
    ...
    for(let enemyTank of level.tanks) {
        enemy = new enemyTanksConv[enemyTank.type](...getPos(enemyTank.pos));
        AItanks.push(enemy);

        extra = createVector(
            (typeof enemyTank.pos.x == "number")? 0.5 * cubeSize.x: 0,
            (typeof enemyTank.pos.y == "number")? 0.5 * cubeSize.y: 0
        );
        enemy.tpRelative(extra);
    }
}
